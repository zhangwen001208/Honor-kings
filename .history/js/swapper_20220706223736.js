...
var imglistEl = swapperEl.querySelector('.imglist')
var adbtnEl = document.querySelector('.adbtn')
var activeEl = adbtnEl.querySelector('.active')

var cuurntIndex = 0 //定义一个记录标识 记录adbtnel的下标
var timer //定义一个定时器标识符
...
adbtnEl.onmouseover = function(event) {
   ...
   //这里计算一个下标 index来拿到 点击那个item就让那个item的下标传给index
    ...
    //这里记录拿到的index传给currentindex在定时器里面 相当于下次就给下标加一
    cuurntIndex = index
    swapercontrol()
 ...
 play()
 //鼠标移入swapperEl定时器取消 离开则继续
swapperEl.onmouseover = function() {
    clearInterval(timer)
}
swapperEl.onmouseout = function() {
    play()
}

function swapercontrol() {
    //这里是改变img的偏移量
    imglistEl.style.transform = `translateX(${- 604 * cuurntIndex}px)`;
    //swapperEl.children[cuurntIndex].style.left  = `${- 604 * cuurntIndex}px` 
